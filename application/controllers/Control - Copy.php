...
class Control extends CI_Controller {
	...
	public function fourth_contents($id=NULL)
	{
		$userID = $this->session->userdata('userID');
		if (isset($userID)) {
			$_SESSION['menu']='home';
			$data['info']=$this->Rest_model->SelectData_1('fourth_page','*',array('id'=>1));
			$this->load->view('admin/fourth_content',$data);
		}else{
			redirect(base_url().'Control', 'refresh');
		}
	}
		public function save_fourth_content()
	{
		$userID = $this->session->userdata('userID');
		if (isset($userID)) {
			$data=$this->input->post();
			...
			$this->Rest_model->UpdateData('fourth_page',$data,array('id'=>$data['id']));
			$this->session->set_flashdata('msg','Data has been updaetd successfully!');
			redirect(base_url().'Control/fourth_contents', 'refresh');
		}else{
			redirect(base_url().'Control', 'refresh');
		}
	}
		...
		public function fourth_seo($id=NULL)
	{
		$userID = $this->session->userdata('userID');
		if (isset($userID)) {
			$_SESSION['menu']='home';
			$data['info']=$this->Rest_model->SelectData_1('fourth_page','*',array('id'=>1));
			$this->load->view('admin/fourth_seo',$data);
		}else{
			redirect(base_url().'Control', 'refresh');
		}
	}
		public function save_fourth_seo()
	{
		$userID = $this->session->userdata('userID');
		if (isset($userID)) {
			$data=$this->input->post();
			...
			$this->Rest_model->UpdateData('fourth_page',$data,array('id'=>$data['id']));
			$this->session->set_flashdata('msg','Data has been updaetd successfully!');
			redirect(base_url().'Control/fourth_seo', 'refresh');
		}else{
			redirect(base_url().'Control', 'refresh');
		}
	}
